...
def knn_testing(model_file, test_file):
    ...
    for test_ind in test_data_img.index:
        # Then for every test image, loop through all training images
        # and calculate the Euclidian distances between each of them
        ...
        # Calculate Euclidean distance between test image and all training images
        # Efficient L2 distance numpy arrays calculation forumula from
        # https://medium.com/@souravdey/l2-distance-matrix-vectorization-trick-26aa3247ac6c by Sourav Dey
        distance_calc = (
            -2 * np.dot(train_data, test_data.T)
            + np.sum(test_data ** 2, axis=1)
            + np.sum(train_data ** 2, axis=1)[:, np.newaxis]
        )
        ...
        distance_tracking = distance_tracking.loc[
            distance_tracking.groupby(0)[2].idxmin()
        ]
    ...
    print(
        ...
        np.round((sum(pred_counts) / len(pred_counts)) * 100, 2),
        "%",
    )
